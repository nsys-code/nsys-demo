...
NSYS_BASEDIR=/opt/nsys-demo
NSYS_HOME=/var/nsys-demo/application-data
NSYS_BUNDLE_FILE=nsys-demo-bundle.zip
NSYS_BUNDLE_PATH=/tmp/$NSYS_BUNDLE_FILE
NSYS_BUNDLE_FORCE_DOWNLOAD=false
NSYS_BUNDLE_SOURCE=/opt/nsys-docker/$NSYS_BUNDLE_FILE
...
checkPrerequisites() {
    ...
    if [ -e $NSYS_BASEDIR/bin/nsys-launcher.sh ] && [ ! "$NSYS_REINSTALL" = "true" ]; then
        echo "Nsys Demo installation detected in folder '$NSYS_BASEDIR'!"
...
createSystemUser() {
    ...
    adduser --system --shell /bin/bash --gecos 'Nsys Demo Control' --ingroup $NSYS_GROUP --disabled-password --home $NSYS_BASEDIR $NSYS_USER
}
...
downloadNsysBundle() {
    ...
    echo
    echo "Downloading Nsys Demo bundle..."
    echo

    cp $NSYS_BUNDLE_SOURCE $NSYS_BUNDLE_PATH
...
installNsysBundle() {
    ...
    if [ "$(ls -A $NSYS_BASEDIR)" ]; then
        ...
        cp /etc/init.d/nsys-demo-daemon $NSYS_BASEDIR/bin/etc/init.d
        cp /etc/init.d/nsys-demo-portal $NSYS_BASEDIR/bin/etc/init.d

        NOW=$(date +"%Y-%m-%d_%H-%M-%S")
        NSYS_BACKUP_FILE="nsys-demo-backup_$NOW.tar.gz"

        echo "Backuping previous installation of the Nsys Demo..."
        echo
    ...
    echo "Installing Nsys Demo bundle to folder '$NSYS_BASEDIR'..."
    echo
    ...
    NSYS_DAEMON_SCRIPT=$NSYS_BASEDIR/bin/nsys-demo-daemon.sh
    sed -i "16s@.*@$(echo $NSYS_RUNAS_SCRIPT)@" $NSYS_DAEMON_SCRIPT
    sed -i "20s@.*@$(echo $NSYS_BASEDIR_SCRIPT)@" $NSYS_DAEMON_SCRIPT

    NSYS_PORTAL_SCRIPT=$NSYS_BASEDIR/bin/nsys-demo-portal.sh
    sed -i "16s@.*@$(echo $NSYS_RUNAS_SCRIPT)@" $NSYS_PORTAL_SCRIPT
    sed -i "20s@.*@$(echo $NSYS_BASEDIR_SCRIPT)@" $NSYS_PORTAL_SCRIPT

    NSYS_DAEMON_CLI_SCRIPT=$NSYS_BASEDIR/bin/nsys-demo-daemon-cli.sh
    sed -i "5s@.*@$(echo $NSYS_BASEDIR_SCRIPT)@" $NSYS_DAEMON_CLI_SCRIPT

    cp $NSYS_BASEDIR/bin/nsys-demo-daemon.sh /etc/init.d/nsys-demo-daemon
    cp $NSYS_BASEDIR/bin/nsys-demo-portal.sh /etc/init.d/nsys-demo-portal
    chmod a+x /etc/init.d/nsys-demo-daemon
    chmod a+x /etc/init.d/nsys-demo-portal

    echo "Nsys Demo has been installed successfully!"
    echo
    echo "To run Nsys Daemon use command:"
    echo "     # /etc/init.d/nsys-demo-daemon run"
    echo
    echo "To run Nsys Portal use command:"
    echo "     # /etc/init.d/nsys-demo-portal run"
    echo
}
...
appHeader() {
    echo
    echo "Nsys Demo Installation Script"
...
appHelp() {
    ...
    echo
    echo "Options:"
    echo "    -u, --nsys-user USER     Nsys Demo Control system user acount"
    echo "    -g, --nsys-group GROUP   Nsys Demo Control system user group"
    echo "    -b, --nsys-basedir DIR   Nsys Demo installation directory (e.g. /opt/nsys-demo)"
    echo "    -h, --nsys-home DIR      Nsys Demo home directory for application data"
    echo "                             (e.g. /var/nsys/application-data)"
    echo "        --reinstall          Uninstall the current version of Nsys Demo,"
    ...
    echo
    echo "Examples:"
    echo "   # $0 --nsys-user nsys-demo --nsys-group nsys-demo \\"
    echo "     --nsys-basedir /opt/nsys-demo --nsys-home /var/nsys-demo/application-data"
    echo "   # $0"
    echo "   # $0 --reinstall"
